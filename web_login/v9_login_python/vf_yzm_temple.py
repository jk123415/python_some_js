...
def web_vf_login(configuration):
    ...
    try:
        ...
        if configuration.get('time_sleep'):
            time.sleep(15)
        else:
            submit = driver.find_element_by_xpath(
                configuration['submit_selector'])
            submit.click()
            time.sleep(7)
        if configuration.get('open_page'):
            driver.get(configuration.get('open_page'))
            time.sleep(7)
...
def web_vf_login_driver(configuration):
    options = webdriver.ChromeOptions()
    if configuration['headless']:
        options.add_argument('--headless')
    driver = webdriver.Chrome(chrome_options=options)
